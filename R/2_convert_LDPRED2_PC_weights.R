...
convert_LDpred2 <- function(trait_type,
  ...
  if (trait_type != "auto" && trait_type != "grid" && trait_type != "outcome") {
    stop("Trait type must be either 'auto', 'grid' or 'outcome'.")
  }
  ...
  if (LDpred2_model == "auto") {
    traits_auto <- as.matrix(fread(file.path(trait_list_dir, paste0("Traits_", LDpred2_model, ".txt")), header = F))

    if (!file.exists(file.path(trait_list_dir, paste0("Traits_", LDpred2_model), "GWAS_LDPRED2_betas.RData"))) {
      ...
      load(file.path(trait_list_dir, paste0("Traits_", LDpred2_model), "GWAS_LDPRED2_betas.RData"))

      ldpred2_beta <- GWAS_LDPRED2_betas
    }
  } else if (LDpred2_model == "grid") {
    traits_grid <- as.matrix(file.path(trait_list_dir, paste0("Traits_", LDpred2_model), ".txt", header = F))
    ...
    if (!file.exists(file.path(trait_list_dir, paste0("Traits_", outcome_db), "GWAS_LDPRED2_betas.RData"))) {
      ldpred2_beta <- collect_LDPRED2_betas(traits_outcome, outcome_db)
    } else {
      load(file.path(trait_list_dir, paste0("Traits_", outcome_db), "/GWAS_LDPRED2_betas.RData"))
      ldpred2_beta <- GWAS_LDPRED2_betas
    }
  }
  ...
  for (set in 1:block[chr])
  {
    ...
    if (trait_type == "outcome") {
      write.table(betas, file = file.path(trait_list_dir, "Betas", paste0(outcome_db, "_LDPRED2_PC_Betas_chr_", chr, "_", set, "_", flag, ".txt")), col.names = T, row.names = F, quote = F, sep = "\t")
    } else {
      write.table(betas, file = file.path(trait_list_dir, "Betas", paste0("GWAS_LDPRED2_PC_Betas_chr_", chr, "_", set, "_", flag, ".txt")), col.names = T, row.names = F, quote = F, sep = "\t")
    }
  }
  return(beta = betas)
}
