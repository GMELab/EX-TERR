...
geno_PCA <- function(id,
 ...
 get_PCA_geno = function(item)
 {
  ...
  load(paste0(rotations_dir, "/G_PC_chr_", chr, "_set_", set, "_id_", group_id, ".RData"))
  #load G_PC_SD
  load(paste0(rotations_dir, "/G_PC_SD_chr_", chr, "_set_", set, "_id_", group_id, ".RData"))
...
for(i in 1:dim(blocks)[1])
{
 ...
 if(!is.null(output_dir)){
  write.table(geno_PCA, paste0(output_dir, "/Geno_PCA_PC_std_threshold_", PC_std_threshold, "_", id, "_", flag, ".txt"), col.names=F, row.names=F, quote=F, sep="\t")
 }
